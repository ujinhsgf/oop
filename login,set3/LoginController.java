...
public class LoginController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        createAccountPane.addEventFilter(MouseEvent.MOUSE_PRESSED, new EventHandler<MouseEvent>() {
            @Override
    ...
    void createAccount(ActionEvent event) {
        ...
        pause.setOnFinished(event1 -> {
            blockPane.setVisible(false);
            //create account here
            String createPassText = createPass.getText();
            String confirmCreatePassText = confirmCreatePass.getText();
            if (createPassText.equals(confirmCreatePassText)) {
                String createUsernameText = createUsername.getText();
                try {
                    FileWriter writer = new FileWriter("login.txt", true);
                    writer.write(createUsernameText + " " + createPassText + "\n");
                    writer.close();
                } catch (IOException e) {
                }
            } else {
